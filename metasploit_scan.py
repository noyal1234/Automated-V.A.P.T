...
class VulnerabilityScanner:
    ...
    def scan_ports_with_nmap(self, target):
        ...
        try:
            ...
            nm.scan(hosts=target, arguments='-T4 -p- -sV')  # Use aggressive timing template
            open_ports = []
    ...
    def fetch_http_vulnerabilities(self, service_version):
        try:
            ...
            api_url = f"http://127.0.0.1:5000/vulnerabilities/{service_version}"
    ...
    def fetch_ftp_vulnerabilities(self, service_version):
        try:
            ...
            api_url = f"http://127.0.0.1:5000/vulnerabilities/ftp+{service_version}"
            response = requests.get(api_url)
            if response.status_code == 200:
                vulnerabilities = response.json()
                logging.info(f"Found vulnerabilities: {vulnerabilities}")
    ...
    def fetch_ssh_vulnerabilities(self, service_version):
        try:
            ...
            api_url = f"http://127.0.0.1:5000/vulnerabilities/ssh+{service_version}"
            response = requests.get(api_url)
            if response.status_code == 200:
                vulnerabilities = response.json()
                logging.info(f"Found vulnerabilities: {vulnerabilities}")
...
if __name__ == "__main__":
    ...
    target = 'http://testhtml5.vulnweb.com'  # Replace with your target IP or URL
